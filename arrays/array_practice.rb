...
puts "If we delete 5, 6, and 7 from the array, we're left with #{array.delete_if{ |number| [5,6,7].include?(number)}}"
puts "If we add 5 at the beginning of the array, we're left with #{array.delete_if{ |number| [5,6,7].include?(number)}.unshift(5)}"
puts "If we add 6 at the end of the array, we're left with #{array.delete_if{ |number| [5,6,7].include?(number)}.unshift(5).push(6)}"
